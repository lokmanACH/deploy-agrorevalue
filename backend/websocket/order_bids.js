function getOrderedBids(productId, users, bids) {
    const bid_array = bids
        ...
        // 3️⃣ sort by quantity (desc), then price (desc), then date (asc)
        .sort((a, b) => {
            // Sort by quantity descending
            if (b.quantity_requested !== a.quantity_requested) {
                return b.quantity_requested - a.quantity_requested;
            }
            // Then by price descending
            if (b.price_per_kg !== a.price_per_kg) {
                return b.price_per_kg - a.price_per_kg;
            }
            // Then by date ascending (earliest first)
