  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        ...
        setBuyer({
          name: `${data.first_name || ""} ${data.last_name || ""}`.trim() || "Utilisateur",
  ...
  return (
    <header className="sticky top-0 z-30 w-full h-16 border-b border-zinc-200 bg-emerald-500/80 backdrop-blur-md dark:border-zinc-800 transition-colors duration-300 flex items-center px-4 sm:px-6 gap-4">
      ...
      <div className="flex items-center gap-2 ml-auto">
        ...
        <button
          onClick={toggleTheme}
          className="flex h-9 w-9 items-center justify-center rounded-full text-white hover:bg-white/20 transition-colors"
        >
          {isDark ? (
        ...
        {/* Notifications */}
        <button className="relative flex h-9 w-9 items-center justify-center rounded-full text-white hover:bg-white/20 transition-colors">
          <Bell className="h-4 w-4" />
          <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-white ring-2 ring-emerald-500" />
        </button>
        ...
        <div className="flex items-center gap-3 pl-2">
          <div className="text-right hidden sm:block">
            ...
            <p className="text-xs text-emerald-100">{buyer.company}</p>
          </div>
          <div className="relative h-10 w-10 overflow-hidden rounded-full border-2 border-white/40 bg-emerald-100">
            <img
