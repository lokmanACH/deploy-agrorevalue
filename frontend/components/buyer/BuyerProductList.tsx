...
interface BuyerProductListProps {
  title: string;
  allowedQualities: Array<"A" | "B" | "C">;
...
export function BuyerProductList({
  ...
  const triggerRefresh = () => setRefresh((current) => !current);
    ...
    async function loadProducts() {
      try {
        ...
        const mappedProducts: Product[] = rawProducts
          .filter(
            (p) => p.status === "active" && allowedQualities.includes(p.quality)
          )
          .map((p) => ({
            id: p.id,
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400">
              {products.length === 0
                ...
                : emptyStateDescription ?? "Essayez un autre terme ou catégorie."}
            </p>
          </div>
        )}
      </div>
