...
export default function BuyersPage() {
  ...
  // ── Derived buyer rows ────────────────────────────────────────────────────
  const baseBuyers = users
    .filter((u) => u.role === "buyer")
    .map((user) => {
      const totalSpend = revenue
      ...
      let status = "Inactif";
      if (user.is_active && user.is_verified) status = "Actif";
      else if (!user.is_verified) status = "En attente";
      ...
      return {
        ...
        spendNumber: totalSpend,
        spend: `${totalSpend.toLocaleString("fr-DZ")} DZD`,
        status,
  ...
  const filteredBuyers = baseBuyers.filter((b) => {
    ...
    const statusMatch = statusFilter === "Tous" || b.status === statusFilter;
    return searchMatch && statusMatch;
  });

  // ── KPI stats ─────────────────────────────────────────────────────────────
  const totalBuyers = baseBuyers.length;
  const enAttente = baseBuyers.filter((b) => b.status === "En attente").length;
  const totalSpendGlobal = baseBuyers.reduce((sum, b) => sum + b.spendNumber, 0);
  const avgMonthlyVolume = totalSpendGlobal / 12;

  const statsCards = [
    {
      name: "Total Acheteurs",
  ...
  const handleExportCSV = () => {
    const rows = [
      ["ID Acheteur", "Nom complet", "Entreprise", "Email", "Téléphone", "Date inscription", "Dépenses (DZD)", "Statut"].join(","),
      ...filteredBuyers.map((b) =>
        [b.id, `"${b.name}"`, `"${b.company}"`, b.email, b.phone, b.date, b.spendNumber, b.status].join(",")
      ),
    ];
    const link = document.createElement("a");
    link.href = "data:text/csv;charset=utf-8," + encodeURIComponent(rows.join("\n"));
    link.download = "acheteurs.csv";
  ...
  const handleSaveStatus = () => {
    if (editingBuyer) {
      ...
      if (editStatusValue == "Actif") {
        api.put<User[]>(`users/${editingBuyer.originalId}`, {
          is_active: true,
          is_verified: true
        })
      }
      ...
      setStatusOverrides((prev) => ({ ...prev, [editingBuyer.originalId]: editStatusValue }));
      setEditingBuyer(null);
      showToast(`Statut de ${editingBuyer.name} mis à jour.`);
    }
  };
  ...
  return (
    <div className="space-y-6 relative">
      {/* Toast */}
      {toast && (
        <div className="fixed bottom-6 right-6 z-[100] flex items-center gap-3 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 px-4 py-3 rounded-xl shadow-2xl text-sm font-medium animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-0.5">
            Consultez et gérez l&apos;ensemble des acheteurs inscrits sur la plateforme.
          </p>
        </div>
        <button
      ...
      <div className="bg-white dark:bg-zinc-950 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-zinc-50 dark:bg-zinc-900/50 border-b border-zinc-200 dark:border-zinc-800 text-zinc-500 dark:text-zinc-400">
              <tr>
                <th className="px-6 py-4 font-medium">ID Acheteur</th>
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
              {loading ? (
                ...
                filteredBuyers.map((buyer) => (
                  <tr key={buyer.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors group">
                    <td className="px-6 py-4 font-mono text-xs font-semibold text-zinc-500 dark:text-zinc-400">{buyer.id}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-zinc-800 dark:text-zinc-200">{buyer.name}</p>
                          <p className="text-xs text-zinc-400 dark:text-zinc-500">{buyer.company}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1">
                        <a
                          ...
                          title={buyer.email}
                          className="text-zinc-400 hover:text-emerald-600 transition-colors p-1.5 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 rounded-full"
                        >
                    ...
                    <td className="px-6 py-4 text-zinc-600 dark:text-zinc-400">{buyer.date}</td>
                    <td className="px-6 py-4 font-medium text-zinc-700 dark:text-zinc-300">{buyer.spend}</td>
                    <td className="px-6 py-4"><StatusBadge status={buyer.status} /></td>
                    <td className="px-6 py-4 text-center">
                      {buyer.licenses.length > 0 ? (
                        <div className="flex justify-center">
                          <button
                            onClick={() => setSelectedLicenses({ urls: buyer.licenses, businessId: buyer.businessId })}
                    ...
                    <td className="px-6 py-4 text-right">
                      <button
                        onClick={() => { setEditingBuyer(buyer); setEditStatusValue(buyer.status); }}
                ...
                <tr>
                  <td colSpan={8} className="px-6 py-16 text-center">
                    <div className="flex flex-col items-center gap-3 text-zinc-400">
                      <Icons.users className="w-10 h-10 opacity-30" />
                      <p className="font-medium">Aucun acheteur trouvé</p>
        ...
        <div className="p-4 border-t border-zinc-200 dark:border-zinc-800 flex items-center justify-between text-sm text-zinc-500 dark:text-zinc-400 bg-zinc-50/50 dark:bg-zinc-900/20">
          <div>
            {loading ? "Chargement…" : `${filteredBuyers.length} résultat${filteredBuyers.length !== 1 ? "s" : ""} sur ${baseBuyers.length} acheteur${baseBuyers.length !== 1 ? "s" : ""}`}
          </div>
          <div className="flex gap-2">
            <button className="px-3 py-1 rounded border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 opacity-40 cursor-not-allowed text-xs">
              Précédent
            </button>
            <button className="px-3 py-1 rounded border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 opacity-40 cursor-not-allowed text-xs">
              Suivant
      ...
      {selectedLicenses && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-zinc-900 rounded-2xl p-6 w-full max-w-lg shadow-2xl relative border border-zinc-200 dark:border-zinc-800">
            <button
              onClick={() => setSelectedLicenses(null)}
              className="absolute top-4 right-4 text-zinc-400 hover:text-zinc-800 dark:hover:text-zinc-100 transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-800 p-1.5 rounded-lg"
            >
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-6">
              N° registre commerce :{" "}
              <span className="font-medium text-zinc-900 dark:text-zinc-100">{selectedLicenses.businessId}</span>
            </p>
              ...
              {selectedLicenses.urls.map((url, i) => (
                <div key={i} className="flex items-center justify-between p-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950">
                  ...
                  <a
                    ...
                    className="text-xs font-medium px-3 py-1.5 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 transition-colors"
                  >
                    Ouvrir
                  </a>
                </div>
            ...
            <div className="flex gap-3 pt-2 border-t border-zinc-200 dark:border-zinc-800">
              <button
                onClick={() => setSelectedLicenses(null)}
                className="flex-1 px-4 py-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg text-sm font-medium text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors"
              >
                Fermer
              </button>
              <button
                onClick={() => handleDownloadAll(selectedLicenses.urls)}
      ...
      {editingBuyer && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-zinc-900 rounded-2xl p-6 w-full max-w-sm shadow-2xl relative border border-zinc-200 dark:border-zinc-800">
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-6">
              Acheteur :{" "}
              <span className="font-medium text-zinc-900 dark:text-zinc-100">{editingBuyer.name}</span>
            </p>
            <div className="mb-6">
              ...
              <select
                ...
                className="w-full bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 text-zinc-900 dark:text-white cursor-pointer"
              >
                <option value="Actif">Actif</option>
                <option value="En attente">En attente</option>
                <option value="Inactif">Inactif</option>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-medium transition-colors shadow-sm"
              >
                Sauvegarder
              </button>
            </div>
