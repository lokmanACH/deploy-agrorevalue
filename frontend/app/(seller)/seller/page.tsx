...
export default function SellerDashboardPage() {
  ...
  const loadSellerProducts = useCallback(async () => {
    try {
      ...
      rawBids.forEach((bid) => {
        if (!bidMap.has(bid.product_id)) {
          bidMap.set(bid.product_id, []);
        }
      ...
      const allocationMap = new Map<
        number,
        (RawAllocation & { buyerName: string })[]
      >();
      ...
      rawAllocations.forEach((alloc) => {
        ...
        if (!allocationMap.has(alloc.product_id)) {
          allocationMap.set(alloc.product_id, []);
        }

        allocationMap.get(alloc.product_id)!.push(allocWithName);
      });

      const mapped: Offer[] = sellerProducts.map((product) =>
        mapToOffer(
          product,
          imageMap,
          locationMap,
      ...
      setError(getErrorMessage(err, "Impossible de charger vos offres"));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const activeProductIdsKey = useMemo(() => {
    return offers
  ...
  useEffect(() => {
    if (!activeProductIdsKey) return;

    const activeProductIds = activeProductIdsKey.split(",").map(Number);
    let isActive = true;
