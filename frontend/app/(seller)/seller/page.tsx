...
//     product.status === "closed" && allocations.length > 0 ? "sold" :
//     product.status === "closed" ? "expired" :
//     "pending";
...
//     status: status as "active" | "pending" | "sold" | "expired",
...
//       <button onClick={onClose} className="ml-2 text-zinc-400 dark:text-zinc-500 hover:text-white dark:hover:text-zinc-900 transition-colors">
//         <X className="w-4 h-4" />
//       </button>
//     </div>
//   );
...
//   // ── Poll active products every 5 seconds ────────────────────────────────────
//   // useEffect(() => {
//   //   if (offers.length === 0) return;

//   //   const activeProductIds = offers
...
//   //   const refreshActiveOffers = async () => {
//   //     try {
...
//   //           const data: BidsResponse = await res.json();
//   //           return data;
//   //         })
//   //       );
...
//   //           if (match.status === "active") nextStatus = "active";
//   //           else if (match.status === "closed" && allocations.length > 0) nextStatus = "sold";
//   //           else if (match.status === "closed") nextStatus = "expired";
...
//   //   refreshActiveOffers();
//   //   intervalId = setInterval(refreshActiveOffers, 5000);
...
//       setOffers((prev) =>
//         prev.map((offer) => {
//           const data = responses.find((r) => r.product_id === offer.id);
//           if (!data) return offer;

//           return {
//             ...offer,
//             offers: data.bids?.length || 0,
...
//             </div>
//             <p className="text-xs text-zinc-400">Vérifiez que le serveur backend est démarré sur le port 5000.</p>
//           </div>
//         )}
...
import { Plus, Package, X, BadgeCheck, Loader2, AlertCircle, Heart } from "lucide-react";
import { AddProductForm } from "@/components/forms/AddProductForm";
import { OffersTab } from "@/components/seller/Offerstab";
...
function mapToOffer(
  ...
  const status =
    product.status === "active"
      ? "active"
      : product.status === "closed" && allocations.length > 0
      ? "sold"
